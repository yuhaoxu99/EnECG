...
class TEMPO(nn.Module):
    ...
    def __init__(self, configs, device):
        ...
        self.time_attn = nn.Linear(768, 1, bias=False)
    ...
    def forward(self, x, itr=0, trend=None, season=None, noise=None, test=False):
        ...
        enc_out = x  # [B, T, 768]  (T = total_patch_num)
        # 通过 time_attn 生成 [B, T, 1] => squeeze => [B, T]
        attn_scores = self.time_attn(enc_out).squeeze(-1)  # 其中 self.time_attn = nn.Linear(768, 1, bias=False)
        # 做 softmax => [B, T], 代表每个时刻的注意力
        ...
        if test:
            return outputs, None, attn_map
        return outputs, loss_local, attn_map
