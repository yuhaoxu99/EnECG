...
class Model(nn.Module):
    ...
    def __init__(self, configs, individual=False):
        ...
        if self.task_name == 'regression':
            self.projection = nn.Linear(self.channels, 1)  # 将通道数聚合为单输出值
    ...
    def reg_encoder(self, x):
        ...
        x = seasonal_output + trend_output
        return x.squeeze(-1)  # 将输出维度从 [B, channels, 1] 调整为 [B, channels]

    def regression(self, x_enc):
        enc_out = self.reg_encoder(x_enc)
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None):
        ...
        if self.task_name == 'classification':
            ...
            return dec_out  # [B, N]
        if self.task_name == 'regression':
            return self.regression(x_enc)  # 返回回归值 [B, 1]
        return None
